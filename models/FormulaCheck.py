...
def formulaCheck(function, fields, numberOfRasterData, numberOfPolygons):
    ...
    possibleRasterAnalysis = ["sum", "mean", "median", "min", "max", "variance", "standDev", "gradientSum",
                              "gradientMin", "gradientMax", "ascent", "descent", "totalClimb", "spSum", "spMean",
                              "spMedian", "spMin", "spMax", "spVariance", "spStandDev", "spGradientSum",
                              "spGradientMin", "spGradientMax","spAscent", "spDescent", "spTotalClimb", "spEuclidean",
                              "spManhattan", "spGeodesic", "spEllipsoidal"]
    comparisonOperators = ["<",">","==",]
    ...
    for i in range(len(formulaParts)):
        ...
        for metric in possibleMetrics:
            if metric in var and len(var) != len(metric):
                ...
                if not(any(res in s for s in possibleMetrics) or res == "True" or res == "False" or res.isnumeric() or\
                       "if" in res or "field:" in res or "math." in res or "raster[" in res or "random" in res or\
                       "polygon[" in var):
                    toReturn = ("Invalid operand", "", 0, len(originalFunction))
                    return toReturn

        if not(any(var in s for s in possibleMetrics) or var == "True" or var == "False" or var.isnumeric() or\
               "if" in var or "field:" in var or "math." in var or "raster[" in var or "random" in var or\
               "polygon[" in var):
            try:
                float(var)
            except:
                if "and" in var:
                    ...
                    for res in andSepParts:
                        if not(any(res in s for s in possibleMetrics) or res == "True" or res == "False" or\
                               res.isnumeric() or "if" in res or "field:" in res or "math." in res or\
                               "raster[" in res or "random" in res or "polygon[" in var):
                            try:
                                float(res)
                            except:
                                toReturn = ("Invalid operand", "", 0, len(originalFunction))
    ...
    while(found):
        ...
        if index != -1:
            ...
            if "if(" in function[index:closingBracketIndex]:
                toReturn = ('Error in if construct: Nested if construct not allowed. Use "and/or" instead', "",
                            errorPos[0], errorPos[1])
                return toReturn
            ...
            for part in ifParts:
                if len(part) == 0:
                    toReturn = ("Error in if construct: Two values in if function necessary", "", errorPos[0],
                                errorPos[1])
                    return toReturn
            ...
            for percentPart in percentParts:
                ...
                if not percent.isnumeric() or int(percent) < 0 or int(percent) > 100:
                    toReturn = ("Error in if construct: No valid integer number as percentage", "", errorPos[0],
                                errorPos[1])
                    return toReturn
            ...
            for andOrPart in andOrSeperatedParts:
                ...
                possOperandsRegex = re.compile(r'field|polygon|math|raster|rnd|random|True|False|euclidean|manhattan|\
                                                geodesic|ellipsoidal')
                res = possOperandsRegex.search(firstOperand)
                if res == None and not firstOperand.isnumeric():
                    toReturn = ("Error in if construct: Invalid first operand of comparison", "", errorPos[0],
                                errorPos[1])
                    return toReturn

                res = possOperandsRegex.search(secondOperand)
                if res == None and not secondOperand.isnumeric() and not("." in secondOperand and\
                   secondOperand.split(".")[0].isnumeric() and secondOperand.split(".")[1].isnumeric):
                    toReturn = ("Error in if construct: Invalid second operand of comparison", "", errorPos[0],
                                errorPos[1])
                    return toReturn
                ...
                if "crossesPolygon" in firstOperand and secondOperand != "True" and secondOperand != "False":
                    toReturn = ("Error in if construct: crossesPolygon can only be compared to False or True", "",
                                errorPos[0], errorPos[1])
                    return toReturn

                if "insidePolygon" in firstOperand and secondOperand != "True" and secondOperand != "False":
                    toReturn = ("Error in if construct: insidePolygon can only be compared to False or True", "",
                                errorPos[0], errorPos[1])
                    return toReturn
            ...
            for sepPart in seperatedParts:
                ...
                if not any(sepPart in s for s in possibleMetrics) and not "math" in sepPart and\
                   not "raster" in sepPart and not "rnd" in sepPart and not "random" in sepPart and\
                   not sepPart.isnumeric():
                    try:
                        float(sepPart)
                    except:
    ...
    while(found):
        ...
        if index != -1:
            ...
            function = function[:index] + function[index:closingBracketIndex-2].replace(",","§") +\
                       function[closingBracketIndex-2:]
            searchMath = re.finditer('math\.[a-z]+\(.+?\)', function[index:closingBracketIndex])
            for matchObj in searchMath:
                function = function[:matchObj.start()+index] +\
                           function[matchObj.start()+index:matchObj.end()+index].replace("§",",") +\
                           function[matchObj.end()+index:]
            if not "§" in function[index:closingBracketIndex] or\
               len(function[index:closingBracketIndex].split("§")) != 2:
                errorPos = __findConstructPosition(originalFunction, "random", partCounter)
            ...
            for value in randomRangeValues:
                ...
                for v in sepParts:
                    v = v.replace("}","")
                    if not v.isnumeric() and not v in possibleMetrics and not "raster[" in v and not "math." in v and\
                       not "if" in v:
                        try:
                            float(v)
                        except:
                            if "," in v:
                                splitRes = v.split(",")
                                for res in splitRes:
                                    if not res.isnumeric() and not res in possibleMetrics and not "raster[" in res and\
                                       not "math." in res and not "if" in res:
                                        try:
                                            float(res)
                                        except:
                                           errorPos = __findConstructPosition(originalFunction, "random", partCounter)
                                           toReturn = ("Error in random function: Invalid upper or lower bound", "",
                                                       errorPos[0], errorPos[1])
                                           return toReturn
                            else:
                                errorPos = __findConstructPosition(originalFunction, "random", partCounter)
        ...
        if res != None:
            ...
            if not "," in number:
                ...
                for n in multiNumbers:
                    if not n in possibleMetrics and not n.isnumeric() and not "field:" in n and not "raster" in n and\
                       not "rnd" in n and not "if" in n and not "and" in n and not n == "True" and not n == "False":
                        try:
                            float(n)
                        except:
                            errorPos = __findConstructPosition(originalFunction, "math", partCounter)
                            toReturn = ("Error in math construct: At least one valid operand necessary", "",
                                        errorPos[0], errorPos[1])
                            return toReturn
            ...
            if "," in number:
                ...
                if len(multNumbers) > 2:
                    errorPos = __findConstructPosition(originalFunction, "math", partCounter)
                    toReturn = ("Error in math construct: Only operations with two variables supported", "",
                                errorPos[0], errorPos[1])
                    return toReturn
                for n1 in multNumbers:
                    n2 = re.split("\+|-|\*|/", n1)
                    for n in n2:
                        if not n.isnumeric() and not "field:" in n and not "raster" in n and not "random" in n and\
                           not "rnd" in n and not "if" in n:
    ...
    for matchString in res:
        if not "[" in matchString:
            ...
            toReturn = ("Error in raster analysis: Index necessary to reference raster data", "", errorPos[0],
                        errorPos[1])
            return toReturn
        partCounter+=1
    ...
    for matchString in res:
        ...
        if not analysisType in possibleRasterAnalysis and not "percentOfValues" in analysisType and\
           not "pixelValue" in analysisType and not "spPercentOfValues" in analysisType and\
           not "spPixelValue" in analysisType:
            errorPos = __findConstructPosition(originalFunction, "raster", partCounter)
    ...
    for spPart in spParts:
        if not "(" in spPart or not ")" in spPart:
            ...
            toReturn = ("Error in raster analysis: Heuristic index for shortest path missing", "", errorPos[0],
                        errorPos[1])
            return toReturn
        ...
        if "," in heuristicIndex:
            ...
            if not percentage.isnumeric() or int(percentage) < 0 or int(percentage) > 100:
                errorPos = __findConstructPosition(originalFunction, "raster\[[0-9]+\]:sp", partCounter)
                toReturn = ("Error in raster analysis: No valid integer number as percentage", "", errorPos[0],
                            errorPos[1])
                return toReturn
        if not heuristicIndex.isnumeric() or int(heuristicIndex) < 0 or int(heuristicIndex) > 5:
            errorPos = __findConstructPosition(originalFunction, "raster\[[0-9]+\]:sp", partCounter)
            toReturn = ("Error in raster analysis: No valid integer number as heuristic index", "", errorPos[0],
                        errorPos[1])
            return toReturn
        partCounter+=1

    percentPixelRegexList = ['raster\[[0-9]+\]:pixelValue', 'raster\[[0-9]+\]:spPixelValue',
                             'raster\[[0-9]+\]:percentOfValues', 'raster\[[0-9]+\]:spPercentOfValues']
    for regex in percentPixelRegexList:
        ...
        for matchObj in findPercentOrPixel:
            if matchObj.start() == 0:
                ...
                toReturn = ("Error in raster analysis: PixelValue analysis can only be used inside if construct", "",
                            errorPos[0], errorPos[1])
                return toReturn
